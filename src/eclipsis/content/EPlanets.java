...
public class EPlanets {
    public static Planet
        chrysidiros = new Planet("chrysidiros", sun, 1f, 2);

    static void initChrysidiros(){
        Planet p = chrysidiros;
        p.generator = new ChrysidirosPlanetGenerator();
        p.meshLoader = () -> new HexMesh(p, 4);
        p.cloudMeshLoader = () -> new MultiMesh(
                new HexSkyMesh(p, 4, 0.8f, 0.64f, 8, new Color().set(Pal.spore).mul(0.9f).a(0.75f), 2, 0.45f, 1f, 0.128f),
                new HexSkyMesh(p, 2, 0.4f, 0.32f, 4, Color.acid.cpy().lerp(Pal.spore, 0.55f).a(0.75f), 2, 0.45f, 1f, 0.64f),
                new HexSkyMesh(p, 1, 0.2f, 0.16f, 2, Color.acid.cpy().lerp(Pal.spore, 0.55f).a(0.75f), 2, 0.45f, 1f, 0.32f)
        );
        p.localizedName = "Chrysidiros";
        p.bloom = true;
        p.visible = true;
        p.accessible = true;
        p.alwaysUnlocked = true;
        p.hasAtmosphere = true;
        p.launchCapacityMultiplier = 0.5f;
        p.sectorSeed = 1337;
        p.allowWaves = true;
        p.allowWaveSimulation = true;
        p.allowSectorInvasion = true;
        p.allowLaunchSchematics = true;
        p.enemyCoreSpawnReplace = true;
        p.allowLaunchLoadout = true;
        //doesn't play well with configs
        p.prebuildBase = false;
        p.ruleSetter = r -> {
            r.waveTeam = Team.malis;
            r.placeRangeCheck = false;
            r.showSpawns = false;
        };
        p.iconColor = Color.acid;
        p.atmosphereColor = Color.acid;
        p.atmosphereRadIn = 0.2f;
        p.atmosphereRadOut = 0.4f;
        p.startSector = 1;
        p.landCloudColor = Color.acid;
        p.hiddenItems.addAll(Items.erekirItems);
        p.hiddenItems.addAll(Items.serpuloItems);
        p.hiddenItems.removeAll(EItems.chrysidirosItems);
        p.defaultCore = EBlocks.coreBit;
    }
    ...
    public static class ChrysidirosPlanetGenerator extends PlanetGenerator{
        ...
        Block[][] arr =
            {
                {Blocks.ferricStone, Blocks.stone, EBlocks.acidicStoneFloor, EBlocks.basaltFloor, EBlocks.acid,
                EBlocks.acidDeep, Blocks.darksand, Blocks.darksand, Blocks.darksand, Blocks.darksand, Blocks.darksand}
            };
...
//            });
            ...
            Seq<Block> ores = Seq.with(EBlocks.oreTin, EBlocks.oreIron, EBlocks.oreLignite);
